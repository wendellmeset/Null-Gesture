...
class DynEnum(Enum):
    ...
    def extend(cls, values):
        ...
        Patched for Python 3.11+ via _add_member_.
        """
        if type(values) is EnumMeta:
            for k in values._member_names_:
                ...
                # Python 3.11+: use proper API for member registration
                if hasattr(cls, '_add_member_'):
                    cls._add_member_(k, v)
                else:
                    # Legacy Python < 3.11 path
